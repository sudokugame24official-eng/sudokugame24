...
const redis = {
  ...
  pipeline: jest
    .fn()
    .mockReturnValue({ zadd: jest.fn(), del: jest.fn(), exec: jest.fn() }),
};
...
describe('P1-Q: period leaderboards', () => {
  ...
  it('window cutoffs scale with the period', async () => {
    ...
    const dailyWhere = (prisma.duelMatch.groupBy as jest.Mock).mock.calls[0][0]
      .where;
    expect(dailyWhere.endTime.gte.getTime()).toBeGreaterThan(
      Date.now() - 2 * 24 * 3600 * 1000,
    );

    await service.getPeriodLeaderboard('yearly', 20);
    const yearlyWhere = (prisma.duelMatch.groupBy as jest.Mock).mock.calls[3][0]
      .where;
    expect(yearlyWhere.endTime.gte.getTime()).toBeLessThan(
  ...
  it('serves from cache when present (no DB hit)', async () => {
    redis.get.mockResolvedValueOnce(
      JSON.stringify([{ rank: 1, userId: 'u1', username: 'x' }]),
    );
  ...
  it('strictly excludes bots and banned users in rebuildLeaderboard', async () => {
    (prisma.profile.findMany as jest.Mock).mockResolvedValue([]);
    await service.rebuildLeaderboard();
  ...
  it('strictly excludes bots and banned users in getTopPlayers fallback', async () => {
    (prisma.profile.findMany as jest.Mock).mockResolvedValue([]);
    await service.getTopPlayers(50, 0);
  ...
  it('strictly excludes bots and banned users in getPeriodLeaderboard profile fetch', async () => {
    redis.get.mockResolvedValue(null);
    (prisma.duelMatch.groupBy as jest.Mock).mockResolvedValue([
      { winnerId: 'u1', _count: { id: 3 } },
    ]);
    (prisma.profile.findMany as jest.Mock).mockResolvedValue([]);

    await service.getPeriodLeaderboard('weekly', 50);
