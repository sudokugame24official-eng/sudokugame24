...
describe('P1-X: DB-driven theme', () => {
  ...
  it('saveDraft merges partial patches', async () => {
    mockRowNull();
    const m = await new ThemeService().saveDraft({
      brandName: 'Mon Sudoku',
      colors: { primary: '10 80% 50%' } as any,
    });
  ...
  it('publish backs up current live for rollback', async () => {
    (prisma.siteSettings.findUnique as jest.Mock).mockImplementation(
      ({ where }) => {
        if (where.key === 'theme_draft')
          return Promise.resolve({
            value: JSON.stringify({ brandName: 'New' }),
          });
        if (where.key === 'theme_published')
          return Promise.resolve({
            value: JSON.stringify({ brandName: 'Old' }),
          });
  ...
  it('rollback without previous -> error', async () => {
    mockRowNull();
    await expect(new ThemeService().rollback()).rejects.toThrow(
