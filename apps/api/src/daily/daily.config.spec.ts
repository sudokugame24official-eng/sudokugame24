...
jest.mock('@repo/sudoku-engine', () => ({
  SudokuGenerator: { generate: jest.fn().mockReturnValue({ initialBoard: Array.from({ length: 9 }, () => Array(9).fill(0)), solvedBoard: Array.from({ length: 9 }, (_, i) => Array(9).fill(i + 1)) }) },
}));
...
const coinLedger = { credit: jest.fn().mockResolvedValue({}), debit: jest.fn() };
...
describe('P1-O: daily challenge admin configurability', () => {
  ...
  it('returns safe defaults when no config is stored', async () => {
    ...
    expect(cfg).toMatchObject({ enabled: true, difficulty: 'MEDIUM', coinRewardPerCell: 5, maxAttempts: 1 });
  });

  it('corrupt stored JSON falls back to defaults (never crashes the daily)', async () => {
    (prisma.siteSettings.findUnique as jest.Mock).mockResolvedValue({ value: '{not json' });
    const cfg = await service.getDailyConfig();
  ...
  it('update merges the patch and persists as JSON', async () => {
    ...
    const cfg = await service.updateDailyConfig({ difficulty: 'EXPERT', coinRewardPerCell: 8 });
    ...
    expect(JSON.parse(call.update.value)).toMatchObject({ difficulty: 'EXPERT' });
  });
  ...
  it('DISABLED daily -> getTodaysChallenge throws (fail-closed)', async () => {
    ...
    await expect(service.getTodaysChallenge()).rejects.toThrow(ForbiddenException);
  });
  ...
  it('auto-generation uses the CONFIGURED difficulty', async () => {
    ...
    (prisma.dailyChallenge.create as jest.Mock).mockImplementation(({ data, include }) => ({
      id: 'c1', date: data.date, puzzle: { solvedBoard: 'X', initialBoard: [] },
    }));

    await service.getTodaysChallenge();

    expect(SudokuGenerator.generate).toHaveBeenCalledWith('HARD');
    const createCall = (prisma.dailyChallenge.create as jest.Mock).mock.calls[0][0];
  ...
  it('publishToday refuses to overwrite an existing challenge (players may have entries)', async () => {
    (prisma.siteSettings.findUnique as jest.Mock).mockResolvedValue(null);
    (prisma.dailyChallenge.findUnique as jest.Mock).mockResolvedValue({ id: 'exists' });
  ...
  it('publishToday creates the challenge with featured/announcement from config', async () => {
    (prisma.siteSettings.findUnique as jest.Mock).mockResolvedValue({
      value: JSON.stringify({ featured: true, announcement: 'Big tournament week!' }),
    });
    (prisma.dailyChallenge.findUnique as jest.Mock).mockResolvedValue(null);
    (prisma.dailyChallenge.create as jest.Mock).mockImplementation(({ data }) => ({
      id: 'c1', date: data.date, puzzle: { initialBoard: [] },
    }));
