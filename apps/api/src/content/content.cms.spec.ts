...
describe('P1-I: CMS workflow', () => {
  ...
  it('updateArticle CANNOT change workflow fields through the generic update', async () => {
    ...
    await service.updateArticle(
      'a1',
      { title: 'ok', status: 'PUBLISHED', publishedAt: new Date() },
      'e',
    );

    // tx.contentArticle.update called inside $transaction — inspect via spy
    const txUsed = await (prisma.$transaction as jest.Mock).mock.results[0]
      .value;
  ...
  it('setStatus SCHEDULED requires a date', async () => {
    ...
    await expect(service.setStatus('a1', 'SCHEDULED', 'e')).rejects.toThrow(
      'date de publication',
    );
  });
  ...
  it('rollback with a wrong revision id is a 404', async () => {
    ...
    await expect(service.rollbackToRevision('a1', 'nope', 'e')).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it('PUBLIC slug endpoint hides non-published and future-scheduled articles', async () => {
    ...
    await expect(service.getArticleBySlug('a1')).rejects.toThrow(
      NotFoundException,
    );
